...
def load_profiles(profiles_dir: Path) -> list[Path]:
    profiles = sorted(profiles_dir.glob("*.ovpn"))
    if not profiles:
        raise VPNError(f"No .ovpn profiles found in {profiles_dir}")
    return profiles
...
def ensure_auth_file(
    ...
    if not username or not password:
        raise VPNError(
            "OpenVPN credentials missing. Provide auth.txt or OPENVPN_USERNAME/OPENVPN_PASSWORD env vars"
        )
...
def connect_vpn(profile: Path, auth_path: Path, run_id: str) -> dict[str, str]:
    """Connects to OpenVPN."""
